...
def f(x,y):
    return np.random.dirichlet(np.ones(x), y)
...
def muestreo_estratificado_mej(df, strata, sample_size=250, seed=None):
    ...
    for i in range(tmp_grpd_ord.shape[0]):
        tmp_df = np.random.choice(indices_dist[i],int(tmp_grpd_ord[i,3]),replace=False)
        lista_aux[i] = tmp_df
